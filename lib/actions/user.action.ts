...
import { getAllUsersProps } from "./shared.types";
...
export async function toggleSaveQuestion(params: {
  userId: string;
  questionId: string;
  path: string;
}) {
...
export async function fetchSavedQuestions(params: {
  ...
  try {
    ...
    const { clerkId, filter, searchQuery, page = 1, pageSize = 5 } = params;
...
export async function getUserInfo(params: { userId: string }) {
  try {
    ...
    const badgeCount = assignBages({ criteria });
